...
import numpy as np
import config
from sequence_labeling.bilstmcrf.bilstm_crf import BiLSTMCRF
...
def prediction(parameters):
    ...
    for test_batch in iter(test_iter):
        ...
        score, pred = model.forward(test_x)
...
def prediction_demo(parameters):
    ...
    for test_batch in iter(test_iter):
        ...
        true = test_batch.target.data.numpy().tolist()
        test_true += true
        score, pred = model.forward(x)
        text_x = x.cpu().data.numpy().tolist()
        test_pred = pred
        result = dh.tokens2text(text_x, test_pred)
        print(result)
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'batch_size': 2,
        'learning_rate': 1e-3,
        'model_path': config.SEQ_LABEL_DIR+'/bilstmcrf/models/news_sample/'
    }
