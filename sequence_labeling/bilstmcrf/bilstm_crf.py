...
class CRF(torch.nn.Module):
    ...
    def __init__(self, in_features, num_tags, start_idx):
        super(CRF, self).__init__()

        self.num_tags = num_tags + 1
        self.start_idx = start_idx
        self.stop_idx = self.num_tags - 1

        self.fc = torch.nn.Linear(in_features, self.num_tags)

        # transition factor, Tij mean transition from j to i
        self.transitions = torch.nn.Parameter(torch.randn(self.num_tags, self.num_tags), requires_grad=True)
